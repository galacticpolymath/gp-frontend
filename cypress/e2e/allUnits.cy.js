...
const liveUnitNumIDs = [];

before(() => {
  cy.request('http://localhost:3000/api/get-units').then((response) => {
    for (const unit of response.body.units) {
      if (unit.PublicationStatus === 'Live' || unit.PublicationStatus === 'Beta') {
        liveUnitNumIDs.push({ numID: unit.numID, locale: unit.locale });
      }
    }
  });
...
describe('Check unit page formatting', () => {
  it('Will check ui of the unit page', () => {
    ...
    const _liveUnitNumIDs = Array.from(liveUnitNumIDs);

    _liveUnitNumIDs.sort((unitNumIDA, unitNumIDB) => unitNumIDA.numID - unitNumIDB.numID);

    for (const unit of _liveUnitNumIDs) {
      testUIOfUnit(unit.numID, unit.locale);
    }
