...
export const addUserToEmailList = async (email, clientUrl) => {
  try {
    ...
    if (!(response.status >= 200 && response.status < 300)) {
      ...
      return {
        wasSuccessful: false,
        errMsg: `Failed to add the user to the mailing list. Response body from Brevo server: ${responseBody}`,
      };
    }

    await deleteMailingListConfirmationsByEmail(email);

    const mailingListConfirmationDoc = new MailingListConfirmation({
      _id: mailingListConfirmationId,
      email,
    });
...
export const deleteUserFromMailingList = async (email) => {
  try {
    ...
    console.error(
      "An error has occurred. Failed to delete the target user from the mailing list.",
      error
    );
...
export const getMailingListContact = async (email) => {
  try {
    if (!email || typeof email !== "string") {
      throw new Error(
        "The email was not provided or was not a string data type."
      );
    }
...
export const getBrevoMailingListContacts = async (
  listIds = [MAILING_LIST_ID],
  offset = 0
) => {
  try {
    ...
    if (response.status !== 200) {
      ...
      console.log(
        "Failed to get the mailing list contacts. Response body: ",
        body
      );
...
export const getAllBrevoMailingListContacts = async (
  ...
  try {
    ...
    const brevoMailingListContacts = await getBrevoMailingListContacts(
      [MAILING_LIST_ID],
      offset
    );

    if (
      !brevoMailingListContacts &&
      typeof brevoMailingListContacts === "object"
    ) {
    ...
    for (const contact of contacts) {
      const isPresent = allContactsClone.find(
        (retrievedContact) => retrievedContact.email === contact.email
      );
    ...
    return await getAllBrevoMailingListContacts(
      offset + BREVO_PG_LIMIT,
      allContactsClone
    );
  } catch (error) {
