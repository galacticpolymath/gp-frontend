...
import { ChevronDown, Download, Eye, FileStack, SquareArrowOutUpRight } from 'lucide-react';
import styles from '../UnitMaterials.module.css';

type TAccordionState = Record<string, boolean>;

type TAssessmentMobileSectionsProps = {
  activeLessonPreviewMode: string;
  openSections: TAccordionState;
  toggleSection: (sectionId: string) => void;
  selectMaterialItemAndPreview: (index: number, itemTitle: string | null | undefined) => void;
  assessmentProps: any;
};
...
const getAssessmentBuckets = (
  items: any[]
): { digital: Array<{ item: any; index: number }>; printed: Array<{ item: any; index: number }> } => {
...
const AssessmentMobileSections: React.FC<TAssessmentMobileSectionsProps> = ({
  ...
  const digitalViewUrl =
    digitalFormViewUrl ?? digitalMaterialUrls.openUrl ?? digitalMaterialUrls.previewUrl ?? null;
    ...
    activeDigitalItem ? (
      <div className={styles.assessmentDigitalWrap}>
        <article
          ...
          onClick={() =>
            selectMaterialItemAndPreview(activeDigitalItem.index, activeDigitalItem.item?.itemTitle)
          }
        >
  ...
  const renderPrintedEntries = (entries: Array<{ item: any; index: number }>) =>
    entries.map(({ item, index }) => {
      const { pdfDownloadUrl, officeDownloadUrl, officeFormat } = assessmentProps.getMaterialUrls(item);
      ...
      const isActive =
        activeLessonPreviewMode === 'materials' && index === assessmentProps.activeMaterialIndex;
      const resourceTitle = formatAssessmentDisplayTitle(item.itemTitle, `Resource ${index + 1}`);
      const itemTypeLabel = (item.itemType ?? item.itemCat ?? '').toString().trim();
      const shouldShowItemTypeLabel = !!itemTypeLabel && !/^assessments?$/i.test(itemTypeLabel);
      const isTeacherOnlyItem =
        typeof item.itemTitle === 'string' && item.itemTitle.toLowerCase().includes('teacher');
      const isTeacherLocked =
        assessmentProps.isAuthenticated && !assessmentProps.isUserTeacher && isTeacherOnlyItem;
      const canAccessPdf =
                  ...
                  {hasOfficeDownload &&
                    (canAccessOffice ? (
                      ...
                      <button
                        ...
                        onClick={() => assessmentProps.handleOpenOfficeUpsell(officeFormat ?? 'Office')}
                      >
                        <span className={styles.materialOfficeLinkContent}>
                          <Image
                            alt="GP+ icon"
  ...
  return [assessmentDigitalSection, assessmentPrintedSection, assessmentGpPlusSection];
};

export default AssessmentMobileSections;
