...
export const LessonItemLink: React.FC<{ href: string, txt: string }> = ({ href, txt = "Your latest copy of this lesson is linked" }) => {
  return (
    <span
  ...
  return (
    <div style={{ width: 'fit-content' }} className={btnWrapperClassName}>
      <Button
        ...
        onClick={onClick}
        style={btnStyles}
        className={`${btnClassName} ${!_didInitialRenderOccurred || disabled || isLoading
          ? 'opacity-25'
          : 'opacity-100'
          }`}
        ...
        callbackFunction: async (data) => {
          if (data?.docs?.length) {
            ...
            if (!fileIds.length) {
              ...
              setIsCopyingLesson(false);
              toast.dismiss(toastId);
              alert('Select one or more files (not folders) to copy this lesson.');
              return;
            }
    ...
    return (
      <div style={{ width: 'fit-content' }} className={btnWrapperClassName}>
        <Button
          ...
          className={`${btnClassName} ${isRetrievingLessonFolderIds ||
            isCopyLessonBtnDisabled ||
            isCopyingLesson
            ? 'opacity-25'
            : 'opacity-100'
