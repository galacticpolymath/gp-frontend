...
export default async function handler(request: NextApiRequest, response: NextApiResponse) {
  try {
    ...
    if (dbFilter && ((typeof dbFilter !== 'object') && (dbFilter === null) || Array.isArray(dbFilter) || (typeof dbFilter !== 'object'))) {
      throw new CustomError('`dbFilter` must be an non-array object. Example: { numID: [1,2,3,4] }', 400);
    }
    const dbFilterEntries = Object.entries(dbFilter as Record<string, unknown[]>);
    const dbFilterCreationResult = (dbFilter && Object.keys(dbFilter).length) ? createDbFilter(dbFilterEntries) : null;
